...
def train():
    ...
    log_dir = os.path.join(args.basedir, args.expname, 'logs', current_time)
    ...
    if args.eval_only:
        # 只进行评估不训练
        def evaluate_models():
            """Evaluate multiple models on test set and log to TensorBoard"""
            # 获取所有要评估的模型路径
            ckpts = [os.path.join(basedir, expname, f) for f in sorted(os.listdir(os.path.join(basedir, expname)))
                     if f.endswith('.tar') and f.split('.')[0].isdigit()]
            ...
            for ckpt_path in ckpts:
                ...
                print(f'\nEvaluating checkpoint at step {step}: {ckpt_path}')
                ...
                # 如果有fine模型也加载
                model_fine = None
                if 'network_fine_state_dict' in ckpt and args.N_importance > 0:
                    model_fine = NeRF(D=args.netdepth_fine, W=args.netwidth_fine,
                                      input_ch=input_ch,
                                      output_ch=output_ch,
                                      skips=[4],
                ...
                with torch.no_grad():
                    ...
                    # 计算指标
                    test_loss = img2mse(torch.tensor(rgbs), test_images)
                    test_psnr = mse2psnr(test_loss)

                    # 记录到TensorBoard
                    writer.add_scalar('eval/loss', test_loss.item(), step)
                    writer.add_scalar('eval/psnr', test_psnr.item(), step)

                    print(
                        f'Step {step}: Loss = {test_loss.item():.4f}, PSNR = {test_psnr.item():.4f}')
    ...
    # For epoch-wise loss calculation
    epoch_losses = []
    current_epoch = 0
    samples_per_epoch = len(i_train) * H * \
        W if use_batching else len(i_train) * args.N_rand
    total_samples = 0
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            ...
            if i_batch >= rays_rgb.shape[0]:
                ...
                if epoch_losses:
                    ...
                    writer.add_scalar('train/epoch_loss',
                                      epoch_loss, current_epoch)
                    tqdm.write(
                        f"[EPOCH] {current_epoch} Avg Loss: {epoch_loss}")
                    epoch_losses = []
                current_epoch += 1
            ...
            if N_rand is not None:
                ...
                select_inds = np.random.choice(coords.shape[0], size=[
                    N_rand], replace=False)  # (N_rand,)
        ...
        # Store loss for epoch calculation
        ...
        if 'rgb0' in extras:
            writer.add_scalar('train/loss0', img_loss0.item(), global_step)
        ...
        if i % args.i_testset == 0 and i > 0:
            ...
            with torch.no_grad():
                # Render test images
                rgbs, disps = render_path(torch.Tensor(poses[i_test]).to(device),
                                          hwf, K, args.chunk, render_kwargs_test,
                                          gt_imgs=images[i_test], savedir=testsavedir)

                # Compute test metrics
                test_loss = img2mse(torch.tensor(
                    rgbs), torch.tensor(images[i_test]))
                test_psnr = mse2psnr(test_loss)

                # Log test metrics
                writer.add_scalar('test/loss', test_loss.item(), global_step)
                writer.add_scalar('test/psnr', test_psnr.item(), global_step)

                # Track best metrics
                if test_psnr.item() > best_psnr:
                    best_psnr = test_psnr.item()
                    best_iter = i
                    writer.add_scalar('test/best_psnr',
                                      best_psnr, global_step)

                if test_loss.item() < best_loss:
                    best_loss = test_loss.item()
                    writer.add_scalar('test/best_loss',
                                      best_loss, global_step)

                print(
                    f'[TEST] Iter: {i} Loss: {test_loss.item()} PSNR: {test_psnr.item()}')
                print(f'Best PSNR: {best_psnr} at iteration {best_iter}')

        # Log validation images and print training progress
        if i % args.i_print == 0:
            tqdm.write(
                f"[TRAIN] Iter: {i} Loss: {loss.item()} PSNR: {psnr.item()}")

            # Log validation images and metrics occasionally
            if i % args.i_img == 0 and i > 0:
                with torch.no_grad():
                    # Render a validation view
                    ...
                    writer.add_scalar(
                        'val/loss', val_loss.item(), global_step)
                    writer.add_scalar(
                        'val/psnr', val_psnr.item(), global_step)

                    # Track best validation metrics
                    if val_psnr.item() > best_val_psnr:
                        best_val_psnr = val_psnr.item()
                        writer.add_scalar(
                            'val/best_psnr', best_val_psnr, global_step)

                    if val_loss.item() < best_val_loss:
                        best_val_loss = val_loss.item()
                        writer.add_scalar(
                            'val/best_loss', best_val_loss, global_step)
        global_step += 1
