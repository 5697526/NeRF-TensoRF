...
def reconstruction(args):
    ...
    for iteration in pbar:
        ...
        # 在 reconstruction 函数中
        # 在 reconstruction 函数中
        if iteration % args.vis_every == args.vis_every - 1 and args.N_vis != 0:
            PSNRs_test, MSEs_test = evaluation(test_dataset, tensorf, args, renderer, f'{logfolder}/imgs_vis/', N_vis=args.N_vis,
                                               prtx=f'{iteration:06d}_', N_samples=nSamples, white_bg=white_bg, ndc_ray=ndc_ray,
                                               writer=summary_writer, global_step=iteration)

            avg_psnr = np.mean(PSNRs_test)
            avg_mse = np.mean(MSEs_test)
            print(f"Test PSNR: {avg_psnr:.2f}, Test MSE: {avg_mse:.6f}")
